...
#Bug: What about there are more than one person paying for the bill?

# Function to get personal items and calculate total cost for each person
def get_personal_items():
    num_people = pyip.inputInt('How many people have personal items on the list?: ', min=1)
    costs = {}
...
def get_shared_items():
    num_items = pyip.inputInt('How many shared items on the list?: ', min=1)
    costs = {}

    for i in range(num_items):
        cost = pyip.inputFloat(f"Enter the cost of shared item {i+1}: ")
        num_people = pyip.inputInt(f"How many people share item {i+1}?: ", min=1)
